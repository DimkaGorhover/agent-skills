...
1. **Version Calculation**: Determines version bump based on commit types
1. **PR Creation**: Creates a "release PR" with updated version and changelog
1. **Release Creation**: When merged, creates a GitHub release and tags
...
1. **Use `feat:` and `fix:`** when you want a release
1. **Use `docs:`, `refactor:`, etc.** when you don't need immediate release
1. **Group related changes** into atomic commits
1. **Think about semver** - does this change API contracts?
1. **Review the release PR** before merging to verify changelog
