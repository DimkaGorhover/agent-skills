...
> **Importing from requirements.txt**: Add each package individually with `uv add <package>`.
> Complex version specifiers and extras may need manual handling — review the file and
> translate each entry rather than bulk-importing, as quoted specifiers and platform markers
> can produce unexpected results.

Then:

1. Delete `requirements.txt`, `requirements-dev.txt`
1. Delete virtual environment (`venv/`, `.venv/`)
1. Add `uv.lock` to version control

### From setup.py / setup.cfg

1. Run `uv init --bare` to create pyproject.toml
1. Use `uv add` to add each dependency from `install_requires`
1. Use `uv add --group dev` for dev dependencies
1. Copy non-dependency metadata (name, version, description, etc.) to `[project]`
1. Delete `setup.py`, `setup.cfg`, `MANIFEST.in`

### From flake8 + black + isort

1. Remove flake8, black, isort via `uv remove`
1. Delete `.flake8`, `pyproject.toml [tool.black]`, `[tool.isort]` configs
1. Add ruff: `uv add --group dev ruff`
1. Add ruff configuration (see [ruff-config.md](./references/ruff-config.md))
1. Run `uv run ruff check --fix .` to apply fixes
1. Run `uv run ruff format .` to format

### From mypy / pyright

1. Remove mypy/pyright via `uv remove`
1. Delete `mypy.ini`, `pyrightconfig.json`, or `[tool.mypy]`/`[tool.pyright]` sections
1. Add ty: `uv add --group dev ty`
1. Run `uv run ty check src/`
