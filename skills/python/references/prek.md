...
1. Remove pre-commit: `pip uninstall pre-commit` or `uv tool uninstall pre-commit`
1. Re-install hooks: `prek install`
1. (Optional) Clean old environments: `rm -rf ~/.cache/pre-commit`
...
1. **Pin hook versions** — Use specific `rev` values, not branches
1. **Use `--cooldown-days` for auto-update** — Mitigates supply chain attacks: `prek auto-update --cooldown-days 7`
1. **Prefer built-in hooks** — Use `repo: builtin` for common checks (faster, offline)
1. **Run hooks before commit** — `prek install` sets this up automatically
1. **Initialize detect-secrets baseline** — Run `detect-secrets scan > .secrets.baseline` before first commit
