...
```bash
# View findings as JSON report (detect-secrets >= 1.4.0)
detect-secrets audit --report .secrets.baseline

# For older versions, inspect the baseline directly
cat .secrets.baseline
```
...
1. Update the package: `uv add <package>@latest`
1. If no fix available: evaluate risk, consider alternatives, or add to ignore list
