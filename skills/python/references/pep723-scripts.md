...
1. **Pin major versions for Python** — Use `>=3.11` not `==3.11`
1. **Omit version constraints for dependencies** — Use `uv add --script` to add dependencies; let uv select versions
1. **Keep scripts focused** — One script, one purpose
1. **Add docstring** — Document what the script does
1. **Use type hints** — Improves readability and catches errors
