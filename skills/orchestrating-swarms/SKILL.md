...
- Coordinating multiple independent agents working on the same codebase in parallel
- Running parallel code reviews (security + performance + architecture simultaneously)
- Creating pipeline workflows where stage N must complete before stage N+1 starts
- Building self-organizing task queues where workers race to claim work items
- Any task that benefits from divide-and-conquer: many files, many reviewers, many independent subtasks

## When NOT to Use

- Tasks that can be completed sequentially in a single agent session — spawning a swarm adds overhead
- Simple searches or one-off reads — use a single `explore` subagent or direct tools instead
- When you don't need results from multiple perspectives — a single focused agent is faster and cheaper
- Tasks with tight shared state or frequent inter-agent coordination — prefer a single agent session
...
1. [Two Ways to Spawn Agents](#two-ways-to-spawn-agents)
1. [Built-in Agent Types](#built-in-agent-types)
1. [Plugin Agent Types](#plugin-agent-types)
1. [TeammateTool Operations](#teammatetool-operations)
1. [Task System Integration](#task-system-integration)
1. [Message Formats](#message-formats)
1. [Orchestration Patterns](#orchestration-patterns)
1. [Environment Variables](#environment-variables)
1. [Spawn Backends](#spawn-backends)
...
1. `$TERM_PROGRAM === "iTerm.app"` or `$ITERM_SESSION_ID` → in iTerm2
1. `which tmux` → tmux available
1. `which it2` → it2 CLI installed
...
1. Their tasks remain in the task list
1. Another teammate can claim their tasks
1. Cleanup will work after timeout expires
